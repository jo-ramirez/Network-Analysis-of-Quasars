...
if __name__ == "__main__":
    ...
    df_phot = pd.DataFrame({
        ...
        # GALEX (UV)
        "FUV_flux": np.array(phot_data["FUV"]),         # nMgy
        "NUV_flux": np.array(phot_data["NUV"]),         # nMgy

        # SDSS (óptico) 
        "u_mag": np.array(phot_data["PSFMAG"])[:, 0],   # mag (AB)
        "g_mag": np.array(phot_data["PSFMAG"])[:, 1],   # mag (AB)
        "r_mag": np.array(phot_data["PSFMAG"])[:, 2],   # mag (AB)
        "i_mag": np.array(phot_data["PSFMAG"])[:, 3],   # mag (AB)
        "z_mag": np.array(phot_data["PSFMAG"])[:, 4],   # mag (AB)

        # 2MASS (NIR) 
        "j_mag": np.array(phot_data["JMAG"]),           # mag (Vega)
        "h_mag": np.array(phot_data["HMAG"]),           # mag (Vega)
        "k_mag": np.array(phot_data["KMAG"]),           # mag (Vega)
        ...
        # WISE (IR medio) 
        "w1_mag": np.array(phot_data["W1_MAG"]),        # mag (vega)
        "w2_mag": np.array(phot_data["W2_MAG"]),        # mag (vega)

        # Calidad de la medicion
        "SN": np.array(phot_data["SN_MEDIAN_ALL"])
    })
